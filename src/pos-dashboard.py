...
df = pd.read_csv('data/WebAssignment.csv', parse_dates={'Date':['YYYY','MM','DD']}, keep_date_col=True)
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            ...
            dcc.Dropdown(id='dropdown5', multi=True, value=['CA'],
...
def updategraph3(cust, year):
    ...
    fig3 = px.bar(df4, x='Quantity', y='CustomerID', orientation='h', hover_name='YYYY', color='Type')
...
def updategraph3(cust, year):
    ...
    fig4 = px.bar(df4, x='Profit', y='CustomerID', orientation='h', hover_name='YYYY', color='Type')
    fig4 = fig4.update_layout(legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.99))
    return fig4
...
def updategraph3(cust, year):
    ...
    fig5 = px.bar(df4, x='Revenue', y='CustomerID', orientation='h', hover_name='YYYY', color='Type')
    fig5 = fig5.update_layout(legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.99))
    return fig5
...
def updategraph3(cust, year):
    ...
    fig6 = px.bar(df4, x='Cost', y='CustomerID', orientation='h', hover_name='YYYY', color='Type')
    fig6 = fig6.update_layout(legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.99))
    return fig6
